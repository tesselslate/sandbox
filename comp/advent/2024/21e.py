...
def all_moves(src, dst, ok):
    ...
    return [x for x in V if manhat(x) == target]
...
def all_move_sequences(code, padkey, start):
    ...
    while len(code) > 0:
        move_set = all_moves(start, pad[code[0]], frozenset(k for k in pad.values()))[0]
        moves = moves + move_set + "A"
        code = code[1:]
...
for code in F:
    ...
    best = None
    for inputs in x:
        if best is None or len(inputs) < len(best):
            best = inputs
