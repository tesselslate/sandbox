...
def all_move_sequences(code, pad, start):
    Q = deque([(start, "", "")])
    V = []

    while len(Q):
        pos, moves, progress = Q.popleft()

        if progress == code:
            V.append(moves)
            continue

        next_char = code[len(progress)]
        for move_set in all_moves(pos, pad[next_char], {k for k in pad.values()}):
            Q.append((pad[next_char], moves + move_set + "A", progress + next_char))
