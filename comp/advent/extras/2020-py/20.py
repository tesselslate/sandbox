...
def build_last_ring():
    ...
    while len(cl) < 4:
        ...
        for x in matches[c].keys():
            ...
            assert False
    edges = list(itertools.chain(itertools.pairwise(cl), [(cl[-1], cl[0])]))
    rings.append(edges)
build_last_ring()
...
def make_rots(ring):
    ...
    def make_grid(ring):
        G = [[None for _ in range(len(ring[0]))] for _ in range(len(ring[0]))]
        for i, x in enumerate(ring[0]):
            G[0][i] = x
        for i, x in enumerate(ring[1]):
            G[i][-1] = x
        for i, x in enumerate(ring[2]):
            G[-1][-i-1] = x
        for i, x in enumerate(ring[3]):
            G[-i-1][0] = x
        return G
    ...
    for i in range(4):
        edges = deque(ring)
        edges.rotate(i)
        rots.append(make_grid(list(edges)))
    return rots

rots = [make_rots(x) for x in rings]
